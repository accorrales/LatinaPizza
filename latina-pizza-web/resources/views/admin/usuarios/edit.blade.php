...
<div class="container mx-auto p-4 max-w-lg">
    <h1 class="text-2xl font-bold mb-6">{{ __('viewAdmin/usuarios_admin.edit.titulo') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.usuarios.update', $usuario['id']) }}">
        ...
        <div class="mb-4">
            <label for="name" class="block text-sm font-semibold mb-1">{{ __('viewAdmin/usuarios_admin.edit.nombre') }}</label>
            <input type="text" name="name" id="name" class="w-full border rounded px-3 py-2" value="{{ $usuario['name'] }}" required>
        </div>

        <div class="mb-4">
            <label for="email" class="block text-sm font-semibold mb-1">{{ __('viewAdmin/usuarios_admin.edit.correo') }}</label>
            <input type="email" name="email" id="email" class="w-full border rounded px-3 py-2" value="{{ $usuario['email'] }}" required>
        </div>

        <div class="mb-4">
            <label for="role" class="block text-sm font-semibold mb-1">{{ __('viewAdmin/usuarios_admin.edit.rol') }}</label>
        ...
        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            {{ __('viewAdmin/usuarios_admin.edit.guardar') }}
        </button>
    </form>
</div>
@endsection
