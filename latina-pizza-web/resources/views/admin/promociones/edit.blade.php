...
<div class="container mx-auto px-4 py-6">
    <h2 class="text-2xl font-bold mb-4">Editar Promoción</h2>
    ...
    <form action="{{ route('admin.promociones.update', $promocion['id']) }}" method="POST">
            ...
            @foreach ($promocion['componentes'] as $i => $componente)
                <div class="componente border p-4 rounded mb-4 bg-gray-50">
                    ...
                    <select name="componentes[{{ $i }}][tipo]" class="w-full border rounded p-2" required>
                        <option value="pizza" {{ $componente['tipo'] == 'pizza' ? 'selected' : '' }}>Pizza</option>
                        <option value="bebida" {{ $componente['tipo'] == 'bebida' ? 'selected' : '' }}>Bebida</option>
                    </select>
        ...
        <div class="mt-6">
            <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded hover:bg-green-700">Actualizar Promoción</button>
