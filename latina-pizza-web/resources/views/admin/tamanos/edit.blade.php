...
<div class="container mx-auto p-6">
    ...
    <form id="formEditarTamano" action="{{ route('admin.tamanos.update', $tamano->id) }}" method="POST" class="space-y-6 bg-white p-6 rounded-lg shadow-md">
        @csrf
        @method('PUT')
        ...
        <div>
            <label for="nombre" class="block font-semibold text-gray-700 mb-1">
                {{ __('viewAdmin/tamanos_admin.edit.nombre') }}
            </label>
        ...
        <div>
            <label for="precio_base" class="block font-semibold text-gray-700 mb-1">
                {{ __('viewAdmin/tamanos_admin.edit.precio_base') }}
            </label>
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('admin.tamanos.index') }}"
               ...
               class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded shadow">
                {{ __('viewAdmin/tamanos_admin.edit.cancelar') }}
            </a>
            <button type="submit" onclick="return validarYMostrarLoading('formEditarTamano')"
                    class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded shadow">
                {{ __('viewAdmin/tamanos_admin.edit.actualizar') }}
            </button>
        </div>
    </form>
</div>
