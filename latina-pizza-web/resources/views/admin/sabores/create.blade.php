...
<div class="container mx-auto p-6">
    ...
    <form id="formCrearSabor" action="{{ route('admin.sabores.store') }}" method="POST"
        ...
        <div>
            <label for="nombre" class="block font-semibold text-gray-700 mb-1">
                {{ __('viewAdmin/sabores_admin.create.nombre') }}
            </label>
        ...
        <div>
            <label for="descripcion" class="block font-semibold text-gray-700 mb-1">
                {{ __('viewAdmin/sabores_admin.create.descripcion') }}
            </label>
        ...
        <div>
            <label for="imagen" class="block font-semibold text-gray-700 mb-1">
                {{ __('viewAdmin/sabores_admin.create.imagen') }}
            </label>
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('admin.sabores.index') }}"
               ...
               class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded shadow">
                {{ __('viewAdmin/sabores_admin.create.cancelar') }}
            </a>
            <button type="submit"
                    onclick="return validarYMostrarLoading('formCrearSabor')"
                    class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded shadow">
                {{ __('viewAdmin/sabores_admin.create.guardar') }}
            </button>
        </div>
    </form>
</div>
