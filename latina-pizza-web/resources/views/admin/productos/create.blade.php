    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(280px,.55fr)]">
        <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <div class="mb-8 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <div class="mb-3 inline-flex items-center gap-2 rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">
                ...
                <a href="{{ route('admin.productos.index') }}" data-show-loading class="inline-flex h-10 items-center gap-2 rounded-full border border-slate-200 px-4 text-xs font-bold text-slate-600 transition hover:bg-slate-50">
            ...
            <form action="{{ route('admin.productos.store') }}" method="POST" class="space-y-6" data-show-loading>
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="nombre" id="nombre" value="{{ old('nombre') }}" required class="w-full rounded-2xl border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-blue-400 focus:ring-blue-400">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="precio" id="precio" value="{{ old('precio') }}" step="0.01" required class="w-full rounded-2xl border-slate-200 bg-slate-50 py-3 pl-9 pr-4 text-sm focus:border-blue-400 focus:ring-blue-400">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="estado" id="estado" class="w-full rounded-2xl border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-400 focus:ring-blue-400">
                            <option value="1" @selected(old('estado', '1') === '1')>{{ __('viewAdmin/productos_admin.create.activo') }}</option>
                            <option value="0" @selected(old('estado') === '0')>{{ __('viewAdmin/productos_admin.create.inactivo') }}</option>
                        </select>
                    </div>
                ...
                <div id="camposPizza" class="hidden rounded-3xl border border-blue-100 bg-[#F6F9FF] p-5 sm:p-6">
                    <div class="mb-5 flex items-center gap-3">
                        ...
                        <div><p class="font-black text-slate-900">Configuración de pizza</p><p class="text-xs text-slate-500">Asigná sabor y tamaño al producto.</p></div>
                    </div>
                    <div class="grid gap-5 md:grid-cols-2">
                        <div>
                            <label for="sabor_id" class="mb-2 block text-sm font-bold text-slate-700">{{ __('viewAdmin/productos_admin.create.sabor') }}</label>
                            <select name="sabor_id" id="sabor_id" class="w-full rounded-2xl border-blue-100 bg-white px-4 py-3 text-sm focus:border-blue-400 focus:ring-blue-400">
                                <option value="">{{ __('viewAdmin/productos_admin.create.seleccionar') }}</option>
                                @foreach ($sabores['data'] ?? $sabores as $sabor)<option value="{{ $sabor['id'] }}" @selected((string) old('sabor_id') === (string) $sabor['id'])>{{ $sabor['nombre'] }}</option>@endforeach
                            </select>
                        </div>
                        <div>
                            <label for="tamano_id" class="mb-2 block text-sm font-bold text-slate-700">{{ __('viewAdmin/productos_admin.create.tamano') }}</label>
                            <select name="tamano_id" id="tamano_id" class="w-full rounded-2xl border-blue-100 bg-white px-4 py-3 text-sm focus:border-blue-400 focus:ring-blue-400">
                                <option value="">{{ __('viewAdmin/productos_admin.create.seleccionar') }}</option>
                                @foreach ($tamanos as $tamano)<option value="{{ $tamano['id'] }}" @selected((string) old('tamano_id') === (string) $tamano['id'])>{{ $tamano['nombre'] }}</option>@endforeach
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col-reverse gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:justify-end">
                    ...
                    <button type="submit" class="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-red-500 px-7 text-sm font-bold text-white shadow-lg shadow-red-950/10 transition hover:-translate-y-0.5 hover:bg-red-600">
                        <i class="fa-solid fa-floppy-disk"></i>{{ __('viewAdmin/productos_admin.create.guardar') }}
                    </button>
                </div>
            </form>
        </section>
        ...
        <aside class="space-y-4">
            <div class="rounded-[2rem] bg-[#071426] p-6 text-white shadow-xl shadow-slate-950/10">
                <div class="grid h-12 w-12 place-items-center rounded-2xl bg-white/10 text-blue-300"><i class="fa-solid fa-lightbulb"></i></div>
                <h2 class="mt-5 text-xl font-black">Tip de catálogo</h2>
                <p class="mt-2 text-sm leading-6 text-slate-300">Usá nombres cortos, una descripción clara y una imagen consistente. El precio de pizzas puede depender del tamaño configurado.</p>
            </div>
            <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <p class="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Flujo</p>
                <div class="mt-4 space-y-3 text-sm text-slate-600">
                    <div class="flex gap-3"><span class="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-blue-50 text-xs font-black text-blue-700">1</span><span>Información básica</span></div>
                    <div class="flex gap-3"><span class="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-blue-50 text-xs font-black text-blue-700">2</span><span>Categoría y estado</span></div>
                    <div class="flex gap-3"><span class="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-red-50 text-xs font-black text-red-600">3</span><span>Configuración especial</span></div>
                </div>
            </div>
        </aside>
    </div>
