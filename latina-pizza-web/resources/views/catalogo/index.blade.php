...
<script>
    ...
    try {
        ...
        // 🎉 Aquí no intentes parsear JSON. Solo mostrar modal si fue exitoso.
        document.getElementById('modalSabor').classList.add('hidden');
        ...
        ocultarLoading(); // ✅ OCULTAR AL FINAL
    }
});
...
<script>
    ...
    function calcularTotalPromo() {
        ...
        document.querySelectorAll(`input[name^="extrasPizza"]`).forEach(input => {
            if (input.checked) {
                totalExtras += parseFloat(input.dataset.precio || 0);
            }
        });
        const total = precioBasePromocion + totalExtras;
        document.getElementById('totalPromo').textContent = `Total: ₡${total.toFixed(2)}`;
    }
    ...
    function agregarPromocionAlCarrito() {
        ...
        while (document.getElementById(`promoSabor${i}_0`)) {
            ...
            while (document.getElementById(`promoSabor${i}_${j}`)) {
                ...
                // ✅ Validación por cada pizza
                if (!saborId || !masaId) {
                    alert(`🍕 Debes seleccionar sabor y masa para la pizza ${i + 1}`);
                    return;
                }
        ...
        if (bebidaSelect && bebidaSelect.value) {
            pizzas.push({
                tipo: 'bebida',
                producto_id: parseInt(bebidaSelect.value)
            });
        }

        // ✅ Payload final
        const payload = {
            promocion_id: promocionSeleccionadaId,
            productos: pizzas
        };
        ...
        .catch(err => {
            ...
            ocultarLoading(); // ✅ OCULTAR LOADING SIEMPRE
        });
    }
