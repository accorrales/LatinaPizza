                    ...
                    @foreach($direcciones as $d)
                        <label class="border rounded-xl p-4 flex gap-3 cursor-pointer hover:shadow transition">
                            ...
                            <div>
                                ...
                                <div class="text-sm text-gray-600">{{ $d['provincia'] }} • {{ $d['canton'] }} • {{ $d['distrito'] }}</div>
                                <div class="text-sm text-gray-600">{{ $d['direccion_exacta'] }}</div>
                                <div class="text-sm text-gray-500">Tel: {{ $d['telefono_contacto'] }}</div>
                                @if(!empty($d['referencias']))<div class="text-xs text-gray-400">Ref: {{ $d['referencias'] }}</div>@endif
                            </div>
                        </label>
                    @endforeach
                </div>
                <button class="px-5 py-2 rounded-xl bg-red-600 text-white font-semibold">Continuar con esta dirección</button>
            </form>
        @else
    ...
    <div class="bg-white rounded-2xl shadow p-5">
        ...
        <form method="POST" action="{{ route('express.store') }}" class="grid md:grid-cols-2 gap-4" id="formNuevaDir">
            ...
            <div class="md:col-span-2">
                <button class="px-5 py-2 rounded-xl bg-gray-900 text-white font-semibold">Guardar dirección</button>
            </div>
        </form>
    </div>
</div>
